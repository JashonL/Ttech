...
import com.tianji.ttech.databinding.ActivityAddPlantBinding
...
class AddPlantActivity : BaseActivity(), View.OnClickListener {
    ...
    companion object {
        ...
        fun start(context: Context?, plantModel: PlantModel? = null) {
            if (TtechApplication.instance().accountService().isGuest()) {
                ToastUtil.show(
                    TtechApplication.instance().getString(R.string.m66_info_space_not_permission)
                )
            } else {
