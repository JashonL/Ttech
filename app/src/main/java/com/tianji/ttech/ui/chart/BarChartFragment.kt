...
class BarChartFragment() : BaseFragment() {
    ...
    private fun showChartData() {
        if (chartListDataModel == null || chartListDataModel!!.getXTimeList()
                .isEmpty() || chartListDataModel!!.getYDataList()
                .isEmpty()
        ) {
        ...
        binding.barChart.xAxis.also {
            ...
            it.valueFormatter = object : ValueFormatter() {
                override fun getFormattedValue(value: Float): String {
                    ...
                    val size = chartListDataModel?.timeList?.size ?: 0
                    if (value.toInt() >= size || value.toInt() < 0) {
                        return ""
                    }
                    return chartListDataModel?.timeList?.get(value.toInt()) ?: ""
                }
            }
        }
